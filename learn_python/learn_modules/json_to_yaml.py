import json
import os
import sys
import yaml

# Checking there is a file name passed
if len(sys.argv) > 1:
    # Opening the source file
    if os.path.exists(sys.argv[1]):
        source_file = open(sys.argv[1], "r")
        source_content = json.load(source_file)
        source_file.close()

        # 1. Convert the JSON to YAML - use yaml library
        converted_yaml_content = yaml.dump(source_content, sort_keys = False, indent = 2)

        # 2.1 Check the target file name was specified as an argument, if not, output the YAML to the screen instead
        # If there are 3 arguments (script to run, file to use, file to save to)
        if len(sys.argv) == 3:
            # 2.2 Check the target file doesn't already exist
            if os.path.exists(sys.argv[2]):
               print("ERROR: " + sys.argv[2] + " already exists, please delete file before trying again.")
            # 2.3 If previous conditions not met, then save YAML file
            else:
                # 2. Save the YAML into a new file with the name for it received as an argument
                with open(sys.argv[2], "w") as yaml_file:
                    yaml_file.write(converted_yaml_content)
        # 2.1 "if not, output the YAML to the screen instead"
        else:
            print(converted_yaml_content)
    # Failing if the file isn't found
    else:
        print("ERROR: " + sys.argv[1] + " not found")
        exit(1)
# No source file specified
else:
    print("ERROR: No JSON file was specified")
    print("Usage: json_to_yaml.py <source_file.json> <target_file.yaml>")